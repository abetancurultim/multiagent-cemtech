...
async function run() {
  console.log("🚀 Iniciando agente de calendario con herramientas Zod...\n");
  ...
  const calendarAgent = createReactAgent({
    ...
    messageModifier: async (messages) => {
      const systemMessage = {
        role: "system",
        content: `Eres un asistente de calendario profesional y amigable que habla en español.
        
Instrucciones:
- Usa las herramientas disponibles para gestionar el calendario
- Siempre confirma las acciones realizadas con detalles específicos
- Si el usuario no especifica duración, asume 1 hora para las reuniones
- Formatea las respuestas de manera clara y profesional
- Usa emojis de manera sutil para hacer las respuestas más amigables`
      };
  ...
  try {
    ...
    console.log("📝 Prueba 1: Creando evento...\n");
    const createResult = await calendarAgent.invoke({
      messages: [{ 
        role: "user", 
        content: "Agenda una reunión técnica con el cliente Jorge Betancur para mañana 12 de Julio de 2025 a las 11:00 AM con duración de 60 minutos. En la descripción añade que revisaremos el sistema de inventarios. Deja en la descripción el email del cliente: jbetancur@unisbc.edu.co, el nombre del cliente es Jorge Betancur y el telefono del cliente es 3178123456." 
      }],
    });
    
    console.log(createResult.messages[createResult.messages.length - 1].content);
    console.log("\n" + "=".repeat(60) + "\n");

    // Prueba 2: Ver eventos
    console.log("📋 Prueba 2: Consultando eventos próximos...\n");
    const viewResult = await calendarAgent.invoke({
      messages: [{ 
        role: "user", 
        content: "¿Qué eventos tengo programados para los próximos 5 días?" 
      }],
    });
    
    console.log(viewResult.messages[viewResult.messages.length - 1].content);
    console.log("\n" + "=".repeat(60) + "\n");

    // Prueba 3: Buscar eventos específicos
    console.log("🔍 Prueba 3: Buscando eventos con Juan...\n");
    const searchResult = await calendarAgent.invoke({
      messages: [{ 
        role: "user", 
        content: "¿Tengo alguna reunión con Juan en el próximo mes?" 
      }],
    });
    
    console.log(searchResult.messages[searchResult.messages.length - 1].content);
