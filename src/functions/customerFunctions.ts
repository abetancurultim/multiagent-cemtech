...
const FAQ_DATABASE = {
  pagos: [
    {
      ...
      answer:
        "Aceptamos: Efectivo, Tarjetas de crédito/débito, Transferencias bancarias, Cheques, Financiación hasta 12 meses sin interés.",
      keywords: ["pago", "formas", "tarjeta", "efectivo", "financiación"],
    },
    {
      question: "¿Ofrecen financiación?",
      answer:
        "Sí, ofrecemos financiación hasta 12 meses sin interés para compras superiores a $2.000.000. También trabajamos con entidades financieras para créditos a mayor plazo.",
  ...
  logistica: [
    {
      ...
      answer:
        "Sí, enviamos a todo el territorio nacional. Tenemos cobertura especial en Antioquia, Córdoba, Chocó, Norte de Santander, Guainía, Boyacá y Arauca.",
      keywords: ["envíos", "colombia", "cobertura", "nacional"],
    },
    {
      question: "¿Cuánto tiempo toma el envío?",
      answer:
        "- Bogotá y área metropolitana: 1-2 días\n- Ciudades principales: 3-5 días\n- Zonas rurales: 5-8 días\n- Instalación incluida: +2 días adicionales",
      keywords: ["tiempo", "envío", "días", "entrega"],
    },
    {
      question: "¿El envío tiene costo?",
      answer:
        "Envío GRATIS para compras superiores a $1.500.000. Para compras menores, el costo varía según destino: $50.000 - $200.000.",
  ...
  garantias: [
    ...
    },
    {
      ...
      answer:
        "La garantía cubre defectos de fabricación, fallas en componentes originales, mano de obra y repuestos. NO cubre daños por mal uso, desgaste normal o modificaciones no autorizadas.",
      keywords: ["cubre", "incluye", "defectos", "repuestos"],
    },
    {
      question: "¿Cómo hacer válida la garantía?",
      answer:
        "Para hacer válida la garantía necesitas: Factura de compra, Descripción del problema, Número de serie del equipo, Contactar al 1-234-5678 ext. 3",
...
const COMPANY_INFO = {
  ...
  description:
    "Empresa líder en equipos industriales para procesamiento de alimentos",
  established: "2010",
...
export function searchFAQ(params: {
  ...
  const categoriesToSearch =
    category === "todas" ? Object.keys(FAQ_DATABASE) : [category];

  for (const cat of categoriesToSearch) {
    const categoryFAQs = FAQ_DATABASE[cat as keyof typeof FAQ_DATABASE];

    if (categoryFAQs) {
      const matches = categoryFAQs.filter(
        (faq) =>
  ...
  results.sort((a, b) => {
    const aScore = a.keywords.filter((keyword: string) =>
      searchQuery.includes(keyword.toLowerCase())
    ).length;
    const bScore = b.keywords.filter((keyword: string) =>
      searchQuery.includes(keyword.toLowerCase())
    ).length;
  ...
  const searchResults = {
    ...
    respuestas: results.map((result) => ({
      ...
      relevancia: result.keywords.filter((keyword: string) =>
        searchQuery.includes(keyword.toLowerCase())
      ).length,
    })),
    sugerencias:
...
export function trackOrder(params: {
  ...
  const order = ORDERS_DATABASE[orderId as keyof typeof ORDERS_DATABASE];
  ...
  const progress =
    statusProgress[order.status as keyof typeof statusProgress] || 0;
...
export function manageComplaint(params: {
  ...
  const complaintInfo =
    COMPLAINT_TYPES[complaintType as keyof typeof COMPLAINT_TYPES];
    ...
    return JSON.stringify(
      {
        ...
        descripcionesTipos: Object.entries(COMPLAINT_TYPES).map(
          ([key, value]) => ({
            tipo: key,
  ...
  const finalPriority =
    priority === "critica" ? "crítica" : complaintInfo.priority;
  ...
  const hours = hoursToAdd[complaintInfo.sla as keyof typeof hoursToAdd] || 24;
  ...
  const complaintRecord = {
    ...
    compensacion:
      finalPriority === "crítica" || complaintType === "entrega_tardia"
 ...
 */
export function getCompanyInfo(infoType?: string): string {
  ...
  const fullInfo = {
    ...
    cobertura: {
      ...
      zonasEspecializadas:
        "Instalación y servicio técnico en zonas de cobertura",
    },
    especialidades: COMPANY_INFO.specialties,
