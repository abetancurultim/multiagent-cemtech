...
const PRODUCTS = {
  PAN001: {
    ...
    category: "panaderia",
    description:
      "Amasadora industrial de alta capacidad para panaderías profesionales",
    specifications: {
      capacity: "80 kg",
      power: "3.5 kW",
  ...
  CHO001: {
    ...
    category: "chocolates",
    description:
      "Templadora automática de chocolate con control de temperatura",
    specifications: {
      capacity: "30 kg/h",
      power: "4.2 kW",
...
export function calculateQuote(params: {
  ...
  const customerDiscount =
    customerDiscounts[customerType as keyof typeof customerDiscounts] || 0;

  // Aplicar descuentos por cantidad
  let quantityDiscount = 0;
  if (quantity >= 5) quantityDiscount = 0.15;
  else if (quantity >= 3) quantityDiscount = 0.1;
  else if (quantity >= 2) quantityDiscount = 0.05;
...
export function getSalesAnalytics(
  ...
  if (productCategory === "todos") {
    ...
    const totalSales = Object.values(salesData).reduce(
      (sum, category) => sum + category.sales,
      0
    );
    ...
    analysis = {
      ...
      resumen: {
        ...
        mejorCrecimiento: categories.reduce((max, cat) =>
          parseInt(cat.crecimiento) > parseInt(max.crecimiento) ? cat : max
        ).categoria,
      },
    ...
    const categoryData = salesData[productCategory as keyof typeof salesData];
  ...
  if (includeComparisons) {
    const prevPeriod =
      timeframe === "mensual" ? "mes anterior" : "trimestre anterior";
 ...
 */
export function getActivePromotions(category?: string): string {
  ...
  let promotions = Object.entries(ACTIVE_PROMOTIONS)
    ...
    .filter(
      ([_, promo]) =>
        !category || category === "todas" || promo.category === category
    )
    .map(([id, promo]) => ({
      id,
      nombre: promo.name,
 ...
 */
export function checkInventory(productId?: string): string {
