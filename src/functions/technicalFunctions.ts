...
export function diagnoseTechnicalIssue(params: {
  ...
  const {
    issueDescription,
    productModel = "Modelo no especificado",
    symptoms = [],
    urgency = "media",
  } = params;
...
export function scheduleTechnicalVisit(params: {
  ...
  const selectedTechnician = availableTechnicians.reduce((best, current) =>
  ...
  const visitInfo = {
    ...
    programacion: {
      ...
      horario:
        urgency === "critica" ? "8:00 AM - 12:00 PM" : "2:00 PM - 6:00 PM",
      duracionEstimada:
        serviceType === "instalacion" ? "2-4 horas" : "1-2 horas",
    },
    preparativos: [
...
export function getTechnicalManual(params: {
  ...
  const manualData =
    TECHNICAL_MANUALS[productModel as keyof typeof TECHNICAL_MANUALS];
  ...
  const manual =
    manualData.manuals[manualType as keyof typeof manualData.manuals];
...
export function checkWarrantyStatus(params: {
  ...
  if (category && WARRANTY_PERIODS[category as keyof typeof WARRANTY_PERIODS]) {
    warrantyInfo = WARRANTY_PERIODS[category as keyof typeof WARRANTY_PERIODS];
  }
...
export function schedulePreventiveMaintenance(params: {
  ...
  console.log(
    "schedulePreventiveMaintenance ejecutado con parámetros:",
    params
  );
  ...
  const specialist =
    Object.values(TECHNICIANS).find(
      (tech) =>
        tech.available &&
        tech.zone === zone &&
    ...
    Object.values(TECHNICIANS).find(
      (tech) => tech.available && tech.zone === zone
    );
